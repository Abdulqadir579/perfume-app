...
def _style_workbook(df: pd.DataFrame) -> BytesIO:
    ...
    new_headers = ["Cost Diff (AED)", "Cost Change %", "Old Margin %",
                   "New Margin %", "Margin \u0394 (pts)", "Price Flag", "Margin Flag"]
    start = ncol + 1
    ...
    for r in range(2, nrow + 1):
        ...
        ws[f'{C["Margin \u0394 (pts)"]}{r}'] = f'=IF(OR({sc}="",{sp}=0),"",(({sp}-{sc})/{sp})-(({sp}-{lc})/{sp}))'
    ...
    money_cols = {col["Last Cost"], col["Supplier Current Cost"], col["Market/Selling Price"], start + 0}
    pct_cols = {start + 1, start + 2, start + 3, start + 4}
    for r in range(2, nrow + 1):
        for c in range(1, total_cols + 1):
    ...
    widths = {"Barcode": 13, "SKU": 10, "Brand": 12, "Perfume Name": 20, "Description": 30,
              ...
              "Match Status": 18, "Cost Diff (AED)": 12, "Cost Change %": 11, "Old Margin %": 11,
